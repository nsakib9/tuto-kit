...
class MessagesController extends Controller
{
    ...
    public function idFetchData(Request $request)
    {
        ...
        // User data
        if ($request['type'] == ('Teacher'||'Student')) {
            $fetch = User::where('id', $request['id'])->first();
        }
        if($request['type'] == 'groups'){
            $fetch = Course::where('id', $request['id'])->first();
        ...
        if($request['type'] == 'group'){
            $fetch = MessengerGroup::where('id', $request['id'])->first();
            return Response::json([
                'favorite' => $favorite,
                'fetch' => $fetch,
            ]);
    ...
    public function fetch(Request $request)
    {
        ...
        if($request->type == 'groups'){
            $query = Chatify::fetchMessagesQueryGroup($request['id'])->orderBy('created_at', 'asc');
            $messages = $query->get();
            if ($query->count() > 0) {
        ...
        if($request->type == 'group'){
            $query = Chatify::fetchMessagesQueryGroupCustom($request['id'])->orderBy('created_at', 'asc');
            $messages = $query->get();
            if ($query->count() > 0) {
                foreach ($messages as $message) {
                    $allMessages .= Chatify::messageCard(
        ...
        if ($users->count() > 0) {
            ...
            foreach ($users as $user) {
                if ($user->id != Auth::user()->id) {
                    ...
                    $data = Chatify::getContactItem($request['messenger_id'], $userCollection);
    ...
    public function search(Request $request)
    {
        ...
        $records = User::where('role_name',ucfirst($request['type']))->where('name', 'LIKE', "%{$input}%")->get();
        $records = getRecords($records);
        foreach ($records as $record) {
            $getRecords .= view('Chatify::layouts.listItem', [
                'get' => 'search_item',
                'type' => $record->role_name,
                'user' => $record,
            ])->render();
        }
        // send the response
        return Response::json([
            'records' => count((Array)$records) > 0
                ? $getRecords
