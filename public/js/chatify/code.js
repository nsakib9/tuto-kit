  ...
  if (messenger != 0) {
    ...
    $.ajax({
      ...
      success: (data) => {
        ...
        console.log(data);
        $('p[id="role_name"]').text(data.fetch.role_name);
...
function sendMessage() {
  ...
  if ($.trim(messageInput.val()).length > 0 || hasFile) {
    ...
    formData.append("id", (messenger.split("_")[0] == "group" ? '77777'+messenger.split("_")[1] : (messenger.split("_")[0] == "groups" ? '88888'+messenger.split("_")[1] : messenger.split("_")[1])) );
...
channel.bind("messaging", function(data) {
  if (data.from_id == messenger.split("_")[1] && data.to_id == auth_id || (data.from_id != auth_id && (data.to_id == '88888'+messenger.split("_")[1] || data.to_id ==  '77777'+messenger.split("_")[1]))) {
    $(".messages")
      .find(".message-hint")
      .remove();
...
channel.bind("client-typing", function(data) {
  if (data.from_id == messenger.split("_")[1] && data.to_id == auth_id || data.to_id == messenger.split("_")[1]) {
    data.typing == true
...
function getContacts() {
  ...
  $.ajax({
    ...
    dataType: "JSON",
    success: (data) => {
      $(".listOfContacts_teacher").html("");
      $(".listOfContacts_student").html("");
      $(".listOfContacts").html("");
      $(".listOfContacts").html(data.contacts);
      
      data.type.forEach(function(item){
        if('Student' == item)
        $(".listOfContacts_student").html(data.student);

        if('Teacher' == item)
        $(".listOfContacts_teacher").html(data.teacher);

      })
      // $(".listOfContacts_student").html("");
      // $(".listOfContacts_student").html(data.contacts);
 ...
 *-------------------------------------------------------------
 */
function messengerSearch(input,type) {
  $.ajax({
    url: url + "/search",
    method: "POST",
    data: { _token: access_token, input: input, type: type},
    dataType: "JSON",
    beforeSend: () => {
...
$(document).ready(function() {
  ...
  $(".messenger-search").on("keyup", function(e) {
    var type = $('body').find('a[class="active-tab"]').data('view')
    $.trim($(this).val()).length > 0
      ? $(".messenger-search").trigger("focus") + messengerSearch($(this).val(),type)
      : $(".messenger-tab").hide() +
