...
@if($get == 'users')
<table id="{{ $type }}" class="messenger-list-item @if($user->id == $id && $id != "0") m-list-active @endif" data-contact="{{ $user->id }}">
    <tr data-action="0">
        {{-- Avatar side --}}
        ...
        <span>
            ...
            <span class="">{{ ucfirst($user->role_name) }}  --</span>
            <span class=""><?php if(!is_teacher() || !is_super()){ echo 'Class -:'; } ?> {{ getClass(Auth::user()->course) }}</span><br><br>
            {!!
