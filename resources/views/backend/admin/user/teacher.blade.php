    ...
    <div class="content">
        ...
        <div class="card">
              <div class="card-header">
                <button id="create" class="btn btn-primary">Create Teacher</button>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  @foreach ($user as $row)
                    <tr>
                      ...
                      <td>{{ $row->email }}</td>
                      <td>{{ $row->phone }}</td>
                      <td><?php if(!is_null($row->expertIn)) {
                        foreach(json_decode($row->expertIn,true) as $item){
                          echo $item.',';
                        }
                        } ?></td>
                      <td>{{ $row->eq }}</td>
...
<form action="{{ url('/admin/user_add?group=teacher') }}" method="post" enctype="multipart/form-data">
    @csrf
<div id="modale_area"></div>
</form>
<form action="{{ url('/admin/user_update?group=teacher') }}" method="post" enctype="multipart/form-data">
    @csrf
...
@section('js')
  <script>      
        $('button[id="create"]').click(function(){
            $.get(base_url+'admin/user_modal?group=teacher',function(data){
                //console.log(data);
        ...
        function edit(id){
          $.get(base_url+'admin/user_modal?group=teacher&id='+id,function(data){
                //console.log(data);
                $('#modale_area').html('');
